...
export async function vectorizeHandler(req: Request & { registry?: ModelRegistry; orchestrator?: FallbackOrchestrator }, res: Response, next: NextFunction): Promise<void> {
  ...
  try {
    ...
    // Stage 2: Use injected singletons (or create per-request fallback)
    const registry = req.registry ?? new ModelRegistry();
    const orchestrator = req.orchestrator ?? new FallbackOrchestrator(registry);
