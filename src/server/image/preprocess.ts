...
/**
 * Quick check if image needs preprocessing based on file size.
 * Returns true only when the buffer exceeds the size threshold —
 * dimension checks require decoding the image and are handled by preprocessImage itself.
 */
export function needsPreprocessing(buffer: Buffer, _maxDimension = 1024, maxFileSize = 5 * 1024 * 1024): boolean {
